...
def parse_html(html):
    soup = BeautifulSoup(
        html)  #使用bs解析获取的页面，测试是可以使用print soup.pretiffy()打印查看获得的页面
    movie_list_soup = soup.find(
        'ol', attrs={'class': 'grid_view'})  #根据css获得要爬取的页面信息
    movie_name_list = []
...
def main():
    url = URL
    with codecs.open('moviesList.txt', 'wb', encoding='utf-8') as fp:  #打开文件，使用utf-8编码
        while url:
